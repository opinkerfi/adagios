...
# will make /adagios/rest/module_name/ available and it loads all functions from 'module_path'
...
for module_name,module_path in rest_modules.items():
    ...
    urlpatterns += patterns('adagios',
        ...
        (base_pattern +'/(?P<format>.+?)/(?P<attribute>.+?)/?$', 'rest.views.handle_request', args),
    )
